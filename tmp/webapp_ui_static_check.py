...
ROOT = Path('/mnt/c/Users/TOT/Documents/MOVEVPE')
...
def run() -> int:
    ...
    with sync_playwright() as p:
        ...
        page_mobile.goto(url, wait_until='domcontentloaded')
        page_mobile.wait_for_timeout(1200)
        header_btn_height = page_mobile.eval_on_selector(
            '#headerResetBtn',
            'el => getComputedStyle(el).minHeight'
        )
        page_mobile.screenshot(path='/tmp/vpe_ui_check_mobile_static.png', full_page=True)
        ...
        browser.close()
        print('SUMMARY: ' + ('FAIL' if failed else 'PASS'))
