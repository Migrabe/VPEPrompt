...
function assertVersionRoutingContracts() {
  ...
  assert(
    ...
    "Tablet user agents must not be forced into the phone shell even when Client Hints report a mobile form factor"
  );
  assert(
...
function assertMobileShellContracts() {
  ...
  const hasDesktopAlternate = /rel="alternate"\s+media="only screen and \(max-width: 640px\)"\s+href="(?:\/mobile\/|mobile\/)"/.test(desktopHtml);
  const hasMobileConfigBase = /window\.VPE_CONFIG_BASE_URL\s*=\s*["'](?:\/config|\.\.\/config)["']/.test(mobileHtml);
  const hasMobileCanonical = /<link rel="canonical" href="(?:\/|\.\.\/)">/.test(mobileHtml);
  const hasSharedRuntimeScripts =
    /<script src="(?:\/js\/|\.\.\/js\/)client_logic_full\.js"><\/script>/.test(mobileHtml) &&
    /<script src="(?:\/js\/|\.\.\/js\/)action-buttons\.js"><\/script>/.test(mobileHtml) &&
    /<script src="(?:\/js\/|\.\.\/js\/)section-visibility\.js"><\/script>/.test(mobileHtml);
  const hasMobileShellScript = /<script src="(?:\/mobile\/js\/|js\/)mobile-shell\.js"><\/script>/.test(mobileHtml);
  const hasSwSharedAssetCoverage =
    /client_logic_full\.js/.test(swSource) &&
    /engine-capabilities\.json/.test(swSource);
  const hasNoLegacyChooserScript =
    !/version-chooser\.js/.test(desktopHtml) &&
    !/version-chooser\.js/.test(mobileHtml) &&
    !/version-chooser\.js/.test(swSource);
  ...
  assert(
    hasNoLegacyChooserScript,
    "Legacy version chooser assets must be removed from desktop, mobile, and service worker shells"
  );
  assert(
    hasMobileConfigBase,
...
async function run() {
  ...
  assert(
    ...
    "NBP JSON prompt must keep reference guidance outside the natural-language scene prompt"
  );
  ...
  assert(!Object.prototype.hasOwnProperty.call(nbp.json || {}, "prompt_format"), "NBP JSON must omit prompt_format");
  assert(!Object.prototype.hasOwnProperty.call(nbp.json || {}, "technical"), "NBP JSON must omit duplicated technical block when root fields already exist");
